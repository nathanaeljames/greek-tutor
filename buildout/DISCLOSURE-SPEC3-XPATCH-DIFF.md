# DISCLOSURE-SPEC3-XPATCH — complete diff

The working-tree diff for the two ports in
[DISCLOSURE-SPEC3-XPATCH.md](DISCLOSURE-SPEC3-XPATCH.md), reported in
[DISCLOSURE-SPEC3-RESULTS-OPUS.md](DISCLOSURE-SPEC3-RESULTS-OPUS.md) §11.
Nothing committed, staged, or pushed — `scripts/ui-disclosure3.mjs` is a new
untracked file, so it is shown below via `git diff --no-index` rather than
`git add -N`, which would have staged it.

## 1. Tracked files — `git diff`

```diff
diff --git a/package.json b/package.json
index 280e080..c6df18d 100644
--- a/package.json
+++ b/package.json
@@ -16,6 +16,7 @@
...
## 2. `scripts/ui-disclosure3.mjs` — adaptation against the Sol original

The informative half of the new file: what the port changed relative to the
copied source. Four hunks, all inside the blocked-autoplay block; every other
line is the Sol script verbatim.

```diff
diff --git sol-space/scripts/ui-disclosure3.mjs opus-space/scripts/ui-disclosure3.mjs
index f1a6e49..20d3fbf 100644
--- sol-space/scripts/ui-disclosure3.mjs
+++ opus-space/scripts/ui-disclosure3.mjs
@@ -365,13 +365,40 @@ for (const entry of exempted) {
   }
 }
...
-// of Chromium's launch policy. The mount request must remain a rendering no-op:
-// item 1 stays visible and the expected rejection is caught without toast or
-// console/page error.
...
@@ -382,8 +409,19 @@ await blockedContext.addInitScript(() => {
     element.paused = true;
     element.ended = false;
     element.play = () => {
-      window.__w2BlockedAttempts += 1;
...
+      return Promise.resolve();
     ...
     element.pause = () => {
       element.paused = true;
@@ -400,6 +438,17 @@ blockedPage.on('pageerror', error => blockedErrors.push(`pageerror: ${error.mess
...
@@ -409,25 +458,50 @@ await blockedPage.waitForFunction(({ expectedHash, expectedTitle }) => {
     && title && clean(title.textContent) === clean(expectedTitle);
 }, {
   expectedHash: blockedHash,
-  expectedTitle: byId.get('c1_learn_letters').activity.title
...
-await blockedPage.waitForFunction(() => window.__w2BlockedAttempts >= 1, null, { timeout: 8000 }).catch(() => {});
...
-const expectedLetter = normalize(chapters.get('chapt_1').alphabet.letters[0].lower);
...
-const blockedAttempts = await blockedPage.evaluate(() => window.__w2BlockedAttempts);
...
-  `prompt ${JSON.stringify(blockedPrompt)}, attempts ${blockedAttempts}`);
...
-  blockedAttempts === 1 && await blockedPage.locator('.toast').count() === 0,
-  `${blockedAttempts} attempts, ${await blockedPage.locator('.toast').count()} toasts`);
