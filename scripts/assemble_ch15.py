...
def endings_topic(tbk, conv):
    ...
    lead = para_blocks(conv, raw.split('\r\n'))[:1]
    ...
    cont = tfield(tbk, 0x27bf2)[0]
    cont_blocks = para_blocks(conv, cont.split('\r\n'))
...
def translation_drill(tbk, conv, txt):
    ...
    # leading blank line is stripped by tpool; the pool then runs entry i =
    # item i for 29 items, with the LAST item's entry absent.
    line2 = tpool(tbk, 0x116a34, n - 1, 'td line 2', allow_blank=True) + ['']
